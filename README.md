...
(`page.typ` → `page.pdf`); rasterize `page.typ` to PNG as above. The card uses
that page's `title` / `subtitle` / `image`, and the page **body is ignored** —
only the metadata reaches the card.
